<?php
/**
 * MQTT Bridge Service for ESP32 Biometric Integration
 * Dragon Edge Group Payroll System
 * Version 2.0 - Fixed SSL Support
 * 
 * Run this continuously: php mqtt_bridge.php
 */

require_once 'db_connection.php';
require __DIR__ . '/vendor/autoload.php';

use PhpMqtt\Client\MqttClient;
use PhpMqtt\Client\ConnectionSettings;

// =====================================================
// CONFIGURATION
// =====================================================

$mqtt_server = "fad64f7d54c740f7b5b3679bdba0f4cf.s1.eu.hivemq.cloud";
$mqtt_port = 8883;
$mqtt_user = "dragonedge";
$mqtt_pass = "DragonEdge2025!";
$mqtt_client_id = "DragonEdge-WebServer-" . uniqid();

// MQTT Topics
$topic_attendance = "dragonedge/attendance";
$topic_enrollment = "dragonedge/enrollment";
$topic_status = "dragonedge/status";

// =====================================================
// LOGGING FUNCTIONS
// =====================================================

function log_message($level, $message) {
    $timestamp = date('Y-m-d H:i:s');
    $log_entry = "[$timestamp] [$level] $message\n";
    echo $log_entry;
    file_put_contents(__DIR__ . '/mqtt_bridge.log', $log_entry, FILE_APPEND);
}

function log_info($message) { log_message('INFO', $message); }
function log_error($message) { log_message('ERROR', $message); }
function log_success($message) { log_message('SUCCESS', $message); }

// =====================================================
// DATABASE HELPER FUNCTIONS
// =====================================================

function get_employee_by_fingerprint($conn, $fingerprint_id) {
    $stmt = $conn->prepare("SELECT * FROM employees WHERE fingerprint_id = ? AND is_active = 1");
    $stmt->bind_param("i", $fingerprint_id);
    $stmt->execute();
    return $stmt->get_result()->fetch_assoc();
}

function update_device_status($conn, $device_id, $status_data) {
    $ip = $status_data['ip'] ?? null;
    $status = $status_data['status'] ?? 'online';
    
    $stmt = $conn->prepare("
        INSERT INTO biometric_devices (device_id, device_name, ip_address, last_seen, status, is_active)
        VALUES (?, ?, ?, NOW(), ?, 1)
        ON DUPLICATE KEY UPDATE
            ip_address = VALUES(ip_address),
            last_seen = NOW(),
            status = VALUES(status)
    ");
    
    $device_name = $device_id;
    $stmt->bind_param("ssss", $device_id, $device_name, $ip, $status);
    return $stmt->execute();
}

// =====================================================
// ATTENDANCE PROCESSING
// =====================================================

function process_attendance($conn, $data) {
    $fingerprint_id = (int)($data['fingerprint_id'] ?? 0);
    $timestamp = $data['timestamp'] ?? date('Y-m-d H:i:s');
    $device_id = $data['device'] ?? 'unknown';
    $is_admin = (bool)($data['is_admin'] ?? false);
    
    log_info("Processing attendance: FP_ID=$fingerprint_id, Time=$timestamp, Device=$device_id");
    
    // 1. Save raw attendance data
    $stmt = $conn->prepare("
        INSERT INTO biometric_attendance_raw 
        (fingerprint_id, device_id, scan_timestamp, is_admin, raw_data, created_at)
        VALUES (?, ?, ?, ?, ?, NOW())
    ");
    $raw_json = json_encode($data);
    $stmt->bind_param("issis", $fingerprint_id, $device_id, $timestamp, $is_admin, $raw_json);
    
    if (!$stmt->execute()) {
        log_error("Failed to save raw attendance: " . $stmt->error);
        return false;
    }
    
    $raw_id = $stmt->insert_id;
    log_info("Raw attendance saved with ID: $raw_id");
    
    // 2. Get employee from fingerprint
    $employee = get_employee_by_fingerprint($conn, $fingerprint_id);
    
    if (!$employee) {
        log_error("Employee not found for fingerprint ID: $fingerprint_id");
        return false;
    }
    
    $employee_id = $employee['id'];
    $employee_name = $employee['first_name'] . ' ' . $employee['last_name'];
    log_info("Employee identified: $employee_name (ID: $employee_id)");
    
    // 3. Process attendance using stored procedure
    try {
        $stmt = $conn->prepare("CALL process_biometric_attendance(?, ?, ?)");
        $stmt->bind_param("iss", $fingerprint_id, $device_id, $timestamp);
        
        if ($stmt->execute()) {
            log_success("✓ Attendance processed for $employee_name");
            return true;
        } else {
            log_error("Stored procedure failed: " . $stmt->error);
            return false;
        }
    } catch (Exception $e) {
        log_error("Exception during attendance processing: " . $e->getMessage());
        return false;
    }
}

// =====================================================
// ENROLLMENT PROCESSING
// =====================================================

function process_enrollment($conn, $data) {
    $employee_id = (int)($data['employee_id'] ?? 0);
    $fingerprint_id = (int)($data['fingerprint_id'] ?? 0);
    $timestamp = $data['timestamp'] ?? date('Y-m-d H:i:s');
    $device_id = $data['device'] ?? 'unknown';
    $enrolled_by = $data['enrolled_by'] ?? 'admin';
    
    log_info("Processing enrollment: Emp_ID=$employee_id, FP_ID=$fingerprint_id");
    
    // 1. Check if employee exists
    $stmt = $conn->prepare("SELECT id, first_name, last_name FROM employees WHERE id = ?");
    $stmt->bind_param("i", $employee_id);
    $stmt->execute();
    $employee = $stmt->get_result()->fetch_assoc();
    
    if (!$employee) {
        log_error("Employee ID $employee_id not found in database");
        return false;
    }
    
    $employee_name = $employee['first_name'] . ' ' . $employee['last_name'];
    
    // 2. Update employee fingerprint_id
    $stmt = $conn->prepare("UPDATE employees SET fingerprint_id = ?, updated_at = NOW() WHERE id = ?");
    $stmt->bind_param("ii", $fingerprint_id, $employee_id);
    
    if (!$stmt->execute()) {
        log_error("Failed to update employee fingerprint: " . $stmt->error);
        return false;
    }
    
    // 3. Record enrollment in biometric_enrollments table
    $stmt = $conn->prepare("
        INSERT INTO biometric_enrollments 
        (employee_id, fingerprint_id, enrolled_by, device_id, enrollment_date)
        VALUES (?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("iisss", $employee_id, $fingerprint_id, $enrolled_by, $device_id, $timestamp);
    
    if ($stmt->execute()) {
        log_success("✓ Employee $employee_name enrolled with Fingerprint ID: $fingerprint_id");
        return true;
    } else {
        log_error("Failed to record enrollment: " . $stmt->error);
        return false;
    }
}

// =====================================================
// MAIN EXECUTION
// =====================================================

log_info("==========================================");
log_info("MQTT Bridge Service Starting...");
log_info("==========================================");

// Test database connection
if (!$conn) {
    log_error("Database connection failed!");
    exit(1);
}
log_success("Database connected");

// Create MQTT client with SSL settings
$connectionSettings = (new ConnectionSettings)
    ->setUsername($mqtt_user)
    ->setPassword($mqtt_pass)
    ->setUseTls(true)
    ->setTlsSelfSignedAllowed(true)  // Allow self-signed certificates
    ->setTlsVerifyPeer(false)         // Don't verify peer (for cloud brokers)
    ->setTlsVerifyPeerName(false)     // Don't verify peer name
    ->setKeepAliveInterval(60)
    ->setConnectTimeout(10)
    ->setSocketTimeout(5)
    ->setResendTimeout(10)
    ->setLastWillTopic($topic_status)
    ->setLastWillMessage(json_encode(['device' => $mqtt_client_id, 'status' => 'offline']))
    ->setLastWillQualityOfService(1);

$mqtt = new MqttClient($mqtt_server, $mqtt_port, $mqtt_client_id);

log_info("Connecting to MQTT broker: $mqtt_server:$mqtt_port");

try {
    $mqtt->connect($connectionSettings, true);
    log_success("MQTT connected successfully");
    
    // Subscribe to topics with message handlers
    $mqtt->subscribe($topic_attendance, function ($topic, $message) use ($conn) {
        log_info("Message received on topic: $topic");
        $data = json_decode($message, true);
        
        if (!$data) {
            log_error("Failed to decode JSON message: $message");
            return;
        }
        
        process_attendance($conn, $data);
    }, 0);
    
    $mqtt->subscribe($topic_enrollment, function ($topic, $message) use ($conn) {
        log_info("Message received on topic: $topic");
        $data = json_decode($message, true);
        
        if (!$data) {
            log_error("Failed to decode JSON message");
            return;
        }
        
        process_enrollment($conn, $data);
    }, 0);
    
    $mqtt->subscribe($topic_status, function ($topic, $message) use ($conn) {
        log_info("Device status update received");
        $data = json_decode($message, true);
        
        if (!$data) {
            return;
        }
        
        $device_id = $data['device'] ?? 'unknown';
        update_device_status($conn, $device_id, $data);
        log_info("Device status updated: $device_id");
    }, 0);
    
    log_success("Subscribed to all topics");
    
    log_info("==========================================");
    log_info("MQTT Bridge running. Waiting for messages...");
    log_info("Press Ctrl+C to stop");
    log_info("==========================================\n");
    
    // Main loop
    $mqtt->loop(true);
    
} catch (Exception $e) {
    log_error("MQTT Error: " . $e->getMessage());
    log_error("Stack trace: " . $e->getTraceAsString());
    exit(1);
}

$mqtt->disconnect();
log_info("MQTT Bridge stopped");